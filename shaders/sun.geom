...
void main() {
	for(int i=0; i<3; i++) {
		// add noise
		// I guess this could actually be done with textures only...
		// but I wanted to try geometry shaders.
		vec4 new_pos      = gl_in[i].gl_Position;
		new_pos += snoise(float(time) * 0.0001 * new_pos.xyz) * normalize(new_pos);

		gl_Position       = he_projection * he_view * model_matrix * new_pos;
		raw_vert          = new_pos;
		world_vertex      = model_matrix * new_pos;
