...
GLuint TextureObject::createCubeTexture(
	...
	// if the exception was thrown, what then?
...
/**
 * Code from: http://content.gpwiki.org/index.php/SDL:Tutorials:Using_SDL_with_OpenGL
 */
...
TextureObject::tex_info TextureObject::getTextureParams(SDL_Surface *surface) {
	...
	if (n_of_colors == 4)  {   // contains an alpha channel
	...
	else if (n_of_colors == 3) {   // no alpha channel
		if (surface->format->Rmask == 0x000000ff) {
